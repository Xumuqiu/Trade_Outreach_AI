...
from app.services.country_catalog_service import get_default_time_zone
...
def create_customer(payload: CustomerCreate, db: Session = Depends(get_db)):
    ...
    derived_tz = payload.time_zone or get_default_time_zone(payload.country)
    customer = Customer(
        name=payload.name,
...
def update_customer(customer_id: int, payload: CustomerUpdate, db: Session = Depends(get_db)):
    ...
    if "country" in data and "time_zone" not in data:
        data["time_zone"] = get_default_time_zone(data.get("country"))
