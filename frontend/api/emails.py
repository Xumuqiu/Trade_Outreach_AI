...
@router.put("/accounts/{account_id}", response_model=EmailAccountOut, status_code=status.HTTP_200_OK)
def update_account(account_id: int, payload: dict, db: Session = Depends(get_db)):
    ...
    if not sender_name or not str(sender_name).strip():
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="sender_name is required")
    try:
        return service.update_account_sender_name(account_id, str(sender_name).strip())
...
def schedule_email(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Schedule failed: {type(exc).__name__}: {exc}",
        )
    return schedule.id
...
def delete_pending_approval_email(email_id: int, db: Session = Depends(get_db)):
    """
    Deletes (soft-delete) an email draft from the approval workflow.

    Only pending_approval emails are eligible so we don't accidentally remove sent/tracked records.
    """
    email = db.query(Email).filter(Email.id == email_id).first()
    if email is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Email not found")
    if email.status != "pending_approval":
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Only pending_approval emails can be deleted",
        )
